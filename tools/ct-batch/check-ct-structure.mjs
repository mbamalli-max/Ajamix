...
const sourcePath = sourceOption
  ...
  : resolve(here, "ct-pilot.json");
const expectedCount = Number(optionValue("--expect-count", "3"));
const expectedLeaf = optionValue("--leaf", "CT03");
...
const expectedIds = Array.from({ length: expectedCount }, (_, index) => `CT${String(index + 1).padStart(2, "0")}`);
...
try {
  ...
  console.error(`check-ct-structure: FAIL — could not read JSON: ${error.message}`);
  process.exit(1);
}
...
for (const [index, module] of (modules || []).entries()) {
  ...
  equal(module.subject, "Critical Thinking", `${where}.subject`);
  equal(module.subjectHa, "Tunani da Tantancewa", `${where}.subjectHa`);
  ...
  equal(module.audioFile, `audio/critical-thinking/${where}.mp3`, `${where}.audioFile`);
...
if (failures.length) {
  console.error(`check-ct-structure: FAIL — ${failures.length} issue(s).`);
...
[
  ...
  `ids CT01–${expectedIds.at(-1)} in order`,
  ...
  `CT01 → … → ${expectedLeaf} chain wiring with ${expectedLeaf} leaf`,
...
].forEach((label) => console.log(`check-ct-structure: PASS — ${label}`));
console.log(`check-ct-structure: PASS — all CT ${expectedCount === 3 ? "pilot" : "candidate"} structure checks passed.`);
