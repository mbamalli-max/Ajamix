...
try {
  ...
  console.error(`check-p5-maths-structure: FAIL: ${error.message}`);
  process.exit(1);
}

if (!Array.isArray(source)) {
  console.error("check-p5-maths-structure: FAIL: source root is not an array.");
  process.exit(1);
}
...
source.forEach((module, index) => {
  ...
  assertEqual(module.gradeband, "p5", `${where}.gradeband`);
...
if (failures.length > 0) {
  console.error(`check-p5-maths-structure: FAIL — ${failures.length} issue(s).`);
  failures.forEach((failure) => console.error(`- ${failure}`));
  process.exit(1);
}

console.log("check-p5-maths-structure: PASS — 24 module(s), exact IDs p5-maths-01..24, schema fields, micro-pauses, quiz counts, assets, and formal leaf fields verified.");
