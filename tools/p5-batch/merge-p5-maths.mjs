...
const batchPath = resolve(here, "p5-maths.json");
const contentPath = resolve(repoRoot, "app", "content.json");
const backupPath = resolve(repoRoot, "app", "content.json.bak");
const expectedIds = Array.from({ length: 24 }, (_, i) => `p5-maths-${String(i + 1).padStart(2, "0")}`);

function fail(message) {
  console.error(`merge-p5-maths: FAIL: ${message}`);
  process.exit(1);
}
...
if (existingBatchIds.length === expectedIds.length) {
  if (originalCount !== 214) fail(`already-merged content count ${originalCount} !== 214`);
  console.log("merge-p5-maths: OK — already merged; no write needed.");
...
if (originalCount !== 190) fail(`original module count ${originalCount} !== 190`);
...
if (merged.modules.length !== 214) fail(`merged module count ${merged.modules.length} !== 214`);
...
console.log("merge-p5-maths: OK — merged.");
console.log(`  original count: ${originalCount}`);
