...
const EXPECTED_ORIGINAL_COUNT = 339;
const EXPECTED_BATCH_COUNT = 15;
const EXPECTED_MERGED_COUNT = 354;
const EXPECTED_IDS = Array.from(
  { length: EXPECTED_BATCH_COUNT },
  (_, index) => `p5-socs-${String(index + 1).padStart(2, '0')}`,
);

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = resolve(scriptDir, '../..');
const batchPath = resolve(scriptDir, 'p5-socs-complete.json');
const contentPath = resolve(repoRoot, 'app/content.json');
const backupPath = resolve(
  repoRoot,
  '../New project/ai-system/projects/ajamix/tasks/2026-07-17-pre-p5-socs-merge-content-339.json',
);
...
const fail = (message) => {
  console.error(`merge-p5-socs: FAIL — ${message}`);
  process.exit(1);
};
...
const sourceModules = readJson(batchPath, 'P5 Social Studies source');
...
// Hard content guard: p5-socs-09 must never carry family-planning content
// (M0 exclusion, re-confirmed at M4 and the consolidated review).
const familyPlanningModule = sourceModules.find((m) => m.id === 'p5-socs-09');
if (familyPlanningModule) {
  const text = JSON.stringify(familyPlanningModule).toLowerCase();
  const bannedTerms = ['tsare haihuwa', 'hana haihuwa', 'shirin iyali', 'family planning', 'contracept'];
  const hit = bannedTerms.find((t) => text.includes(t));
  if (hit) fail(`p5-socs-09 contains banned family-planning term "${hit}" — merge blocked`);
}
...
if (existingBatchIds.length === EXPECTED_BATCH_COUNT) {
  ...
  console.log(`merge-p5-socs: OK — already merged; ${EXPECTED_MERGED_COUNT} modules, exact source match.`);
  process.exit(0);
}
...
if (liveContent.modules.length !== EXPECTED_ORIGINAL_COUNT) {
  fail(`live count is ${liveContent.modules.length}; expected ${EXPECTED_ORIGINAL_COUNT} before merge (did P4 Social Studies merge run first?)`);
}
...
console.log(
  `merge-p5-socs: OK — ${EXPECTED_ORIGINAL_COUNT} -> ${EXPECTED_MERGED_COUNT}; appended ${EXPECTED_IDS[0]}..${EXPECTED_IDS.at(-1)}; preserved all existing modules and non-module fields; backup=${backupPath}`,
);
