...
const sourcePath = resolve(here, "p5-maths.json");
const expectedIds = Array.from({ length: 24 }, (_, index) => `p5-maths-${String(index + 1).padStart(2, "0")}`);
const safeFormulaPattern = /^[\d\s+\-*/().]+$/;
const randomIterations = 100;
const minimumGeneratedValue = 0;
const maximumGeneratedValue = 100000;
const failures = [];

function fail(message) {
  console.error(`check-p5-maths-formulas: FAIL: ${message}`);
  process.exit(1);
}
...
function assertValues(moduleId, questionIndex, variables, answer, distractors) {
  ...
  if (answer > maximumGeneratedValue) {
    recordFailure(moduleId, questionIndex, variables, answer, distractors, "answer above 100,000");
  }
  if (distractors.some((value) => value > maximumGeneratedValue)) {
    recordFailure(moduleId, questionIndex, variables, answer, distractors, "distractor above 100,000");
  }
...
if (failures.length > 0) {
  console.error(`check-p5-maths-formulas: FAIL — ${failures.length} formula issue(s).`);
...
console.log(
  `check-p5-maths-formulas: PASS — ${source.length} module(s), ${questionCount} quiz question(s), ${deterministicCaseCount} deterministic edge case(s), ${randomCaseCount} random case(s), ${totalCaseCount} total case(s); all generated answers/distractors stayed within 0–100,000.`
);
