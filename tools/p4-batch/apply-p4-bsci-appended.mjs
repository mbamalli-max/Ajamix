...
const newPath = resolve(here, "p4-bsci-appended-new.json");
const contentPath = resolve(repoRoot, "app", "content.json");
const backupPath = resolve(repoRoot, "app", "content.json.bak");

const expectedExistingIds = Array.from({ length: 15 }, (_, i) => `p4-bsci-${String(i + 1).padStart(2, "0")}`);
const expectedNewIds = ["p4-bsci-16", "p4-bsci-17", "p4-bsci-18", "p4-bsci-19", "p4-bsci-20", "p4-bsci-21"];

function fail(message) {
  console.error(`apply-p4-bsci-appended: FAIL: ${message}`);
  process.exit(1);
}
...
if (newIdsAlreadyPresent.length === expectedNewIds.length) {
  if (originalCount !== 279) fail(`already-applied content count ${originalCount} !== 279`);
  console.log("apply-p4-bsci-appended: OK — already applied; no write needed.");
...
if (originalCount !== 273) fail(`original module count ${originalCount} !== 273`);
...
if (JSON.stringify(appended.map((m) => m.id)) !== JSON.stringify(expectedNewIds)) {
  fail(`new-module ids are not exactly the expected six, in order: ${appended.map((m) => m.id).join(", ")}`);
}
...
if (merged.modules.length !== 279) fail(`merged module count ${merged.modules.length} !== 279`);
...
console.log("apply-p4-bsci-appended: OK — applied.");
console.log(`  original count: ${originalCount}`);
