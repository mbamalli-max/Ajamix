...
const batchPath = resolve(here, "p4-bsci.json");
const contentPath = resolve(repoRoot, "app", "content.json");
const backupPath = resolve(repoRoot, "app", "content.json.bak");
const expectedIds = Array.from({ length: 15 }, (_, i) => `p4-bsci-${String(i + 1).padStart(2, "0")}`);

function fail(message) {
  console.error(`merge-p4-bsci: FAIL: ${message}`);
  process.exit(1);
}
...
if (existingBatchIds.length === expectedIds.length) {
  if (originalCount !== 268) fail(`already-merged content count ${originalCount} !== 268`);
  console.log("merge-p4-bsci: OK — already merged; no write needed.");
...
if (originalCount !== 253) fail(`original module count ${originalCount} !== 253`);
...
if (merged.modules.length !== 268) fail(`merged module count ${merged.modules.length} !== 268`);
...
console.log("merge-p4-bsci: OK — merged.");
console.log(`  original count: ${originalCount}`);
