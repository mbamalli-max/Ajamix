...
const maximumGeneratedValue = 10000;
const failures = [];

function fail(message) {
  console.error(`check-p4-maths-formulas: FAIL: ${message}`);
  process.exit(1);
}
...
function assertValues(moduleId, questionIndex, variables, answer, distractors) {
  ...
  if (answer > maximumGeneratedValue) {
    recordFailure(moduleId, questionIndex, variables, answer, distractors, "answer above 10,000");
  }
  if (distractors.some((value) => value > maximumGeneratedValue)) {
    recordFailure(moduleId, questionIndex, variables, answer, distractors, "distractor above 10,000");
  }
...
if (failures.length > 0) {
  console.error(`check-p4-maths-formulas: FAIL — ${failures.length} formula issue(s).`);
...
console.log(
  `check-p4-maths-formulas: PASS — ${source.length} module(s), ${questionCount} quiz question(s), ${deterministicCaseCount} deterministic edge case(s), ${randomCaseCount} random case(s), ${totalCaseCount} total case(s); all generated answers/distractors stayed within 0–10,000.`
);
