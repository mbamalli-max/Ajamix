...
const batchPath = resolve(here, "p3-bsci-m2.json");
const contentPath = resolve(repoRoot, "app", "content.json");
const backupPath = resolve(repoRoot, "app", "content.json.bak");
const expectedIds = Array.from({ length: 9 }, (_, i) => `p3-bsci-${String(i + 7).padStart(2, "0")}`);

function fail(message) {
  console.error(`merge-p3-bsci-m2: FAIL: ${message}`);
  process.exit(1);
}
...
if (existingBatchIds.length === expectedIds.length) {
  if (originalCount !== 253) fail(`already-merged content count ${originalCount} !== 253`);
  console.log("merge-p3-bsci-m2: OK — already merged; no write needed.");
...
if (originalCount !== 244) fail(`original module count ${originalCount} !== 244`);
...
if (merged.modules.length !== 253) fail(`merged module count ${merged.modules.length} !== 253`);
...
console.log("merge-p3-bsci-m2: OK — merged.");
console.log(`  original count: ${originalCount}`);
