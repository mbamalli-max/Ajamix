...
const batchPath = resolve(here, "p3-bsci.json");
const contentPath = resolve(repoRoot, "app", "content.json");
const backupPath = resolve(repoRoot, "app", "content.json.bak");
const expectedIds = Array.from({ length: 6 }, (_, i) => `p3-bsci-${String(i + 1).padStart(2, "0")}`);

function fail(message) {
  console.error(`merge-p3-bsci-pilot: FAIL: ${message}`);
  process.exit(1);
}
...
if (existingBatchIds.length === expectedIds.length) {
  if (originalCount !== 244) fail(`already-merged content count ${originalCount} !== 244`);
  console.log("merge-p3-bsci-pilot: OK — already merged; no write needed.");
...
if (originalCount !== 238) fail(`original module count ${originalCount} !== 238`);
...
if (merged.modules.length !== 244) fail(`merged module count ${merged.modules.length} !== 244`);
...
console.log("merge-p3-bsci-pilot: OK — merged.");
console.log(`  original count: ${originalCount}`);
