...
function fail(message) {
  console.error(`merge-p3-maths-pilot: FAIL: ${message}`);
  process.exit(1);
}
...
if (existingBatchIds.length === expectedIds.length) {
  if (originalCount !== 148) fail(`already-merged content count ${originalCount} !== 148`);
  console.log("merge-p3-maths-pilot: OK — already merged; no write needed.");
...
if (originalCount !== 142) fail(`original module count ${originalCount} !== 142`);
...
if (merged.modules.length !== 148) fail(`merged module count ${merged.modules.length} !== 148`);
...
console.log("merge-p3-maths-pilot: OK — merged.");
console.log(`  original count: ${originalCount}`);
