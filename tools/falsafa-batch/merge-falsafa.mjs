...
const EXPECTED_ORIGINAL_COUNT = 369;
const EXPECTED_BATCH_COUNT = 10;
const EXPECTED_MERGED_COUNT = 379;
const EXPECTED_IDS = Array.from(
  { length: EXPECTED_BATCH_COUNT },
  (_, index) => `FL${String(index + 1).padStart(2, '0')}`,
);

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = resolve(scriptDir, '../..');
const batchPath = resolve(scriptDir, 'falsafa-complete.json');
const contentPath = resolve(repoRoot, 'app/content.json');
const backupPath = resolve(
  repoRoot,
  '../New project/ai-system/projects/ajamix/tasks/2026-07-25-pre-falsafa-merge-content-369.json',
);

// Derived directly from tools/ct-batch/ct-complete.json and the live V01
// vocational module; Falsafa uses the adult/vocational schema, not the
// shorter formal-track schema used by the Social Studies merge script.
const KEY_ORDER = [
...
const fail = (message) => {
  console.error(`merge-falsafa: FAIL — ${message}`);
  process.exit(1);
};
...
const sourceModules = readJson(batchPath, 'Falsafa source');
...
if (religiousHit) {
  fail(`religious-sensitivity guard found banned term ${JSON.stringify(religiousHit)} in the Falsafa source`);
}

// FL04 legitimately uses "wuta" in an electrical-circuit lesson. Any source
// module using it must include a concrete circuit-context word; otherwise the
// intended sense is uncertain and the merge must stop for human review.
for (const module of sourceModules) {
  const moduleText = JSON.stringify(module).toLocaleLowerCase('ha');
  if (moduleText.includes('wuta') && !/(fitila|maballi)/u.test(moduleText)) {
    fail(`religious-sensitivity guard found "wuta" without fitila/maballi electrical-circuit context in ${module.id}`);
  }
}
...
if (existingBatchIds.length === EXPECTED_BATCH_COUNT) {
  ...
  console.log('merge-falsafa: PASS — religious-sensitivity guard found no banned religious terms; FL04 "wuta" is in fitila/maballi electrical-circuit context.');
  console.log(`merge-falsafa: OK — already merged; ${EXPECTED_MERGED_COUNT} modules, exact source match.`);
  process.exit(0);
}
...
console.log('merge-falsafa: PASS — religious-sensitivity guard found no banned religious terms; FL04 "wuta" is in fitila/maballi electrical-circuit context.');
console.log(
  `merge-falsafa: OK — ${EXPECTED_ORIGINAL_COUNT} -> ${EXPECTED_MERGED_COUNT}; appended ${EXPECTED_IDS[0]}..${EXPECTED_IDS.at(-1)}; preserved all existing modules and non-module fields; backup=${backupPath}`,
);
