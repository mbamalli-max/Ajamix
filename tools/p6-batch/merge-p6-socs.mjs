...
const EXPECTED_ORIGINAL_COUNT = 354;
const EXPECTED_BATCH_COUNT = 15;
const EXPECTED_MERGED_COUNT = 369;
const EXPECTED_IDS = Array.from(
  { length: EXPECTED_BATCH_COUNT },
  (_, index) => `p6-socs-${String(index + 1).padStart(2, '0')}`,
);

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = resolve(scriptDir, '../..');
const batchPath = resolve(scriptDir, 'p6-socs-complete.json');
const contentPath = resolve(repoRoot, 'app/content.json');
const backupPath = resolve(
  repoRoot,
  '../New project/ai-system/projects/ajamix/tasks/2026-07-17-pre-p6-socs-merge-content-354.json',
);
...
const fail = (message) => {
  console.error(`merge-p6-socs: FAIL — ${message}`);
  process.exit(1);
};
...
const sourceModules = readJson(batchPath, 'P6 Social Studies source');
...
// Hard content guard: p6-socs-09 must never carry terrorism/extremist-group
// framing (brief §7 absolute prohibition, re-confirmed at M5 and the
// consolidated review). Title must also stay the approved safety framing.
const safetyModule = sourceModules.find((m) => m.id === 'p6-socs-09');
if (safetyModule) {
  const text = JSON.stringify(safetyModule).toLowerCase();
  const bannedTerms = [
    'ta\'addanci', 'terrorism', 'terrorist', 'boko haram', 'isis', 'al-qaeda',
    'militant', 'jihad', 'insurgen', 'extremist',
  ];
  const hit = bannedTerms.find((t) => text.includes(t));
  if (hit) fail(`p6-socs-09 contains banned term "${hit}" — merge blocked`);
  if (safetyModule.titleEn !== 'Safety and Security — Staying Safe as a Citizen') {
    fail(`p6-socs-09 titleEn has drifted from the approved title: "${safetyModule.titleEn}"`);
  }
}
...
if (existingBatchIds.length === EXPECTED_BATCH_COUNT) {
  ...
  console.log(`merge-p6-socs: OK — already merged; ${EXPECTED_MERGED_COUNT} modules, exact source match.`);
  process.exit(0);
}
...
if (liveContent.modules.length !== EXPECTED_ORIGINAL_COUNT) {
  fail(`live count is ${liveContent.modules.length}; expected ${EXPECTED_ORIGINAL_COUNT} before merge (did P5 Social Studies merge run first?)`);
}
...
console.log(
  `merge-p6-socs: OK — ${EXPECTED_ORIGINAL_COUNT} -> ${EXPECTED_MERGED_COUNT}; appended ${EXPECTED_IDS[0]}..${EXPECTED_IDS.at(-1)}; preserved all existing modules and non-module fields; backup=${backupPath}`,
);
