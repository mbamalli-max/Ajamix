...
const batchPath = resolve(here, "p6-maths.json");
const contentPath = resolve(repoRoot, "app", "content.json");
const backupPath = resolve(repoRoot, "app", "content.json.bak");
const expectedIds = Array.from({ length: 24 }, (_, i) => `p6-maths-${String(i + 1).padStart(2, "0")}`);

function fail(message) {
  console.error(`merge-p6-maths: FAIL: ${message}`);
  process.exit(1);
}
...
if (existingBatchIds.length === expectedIds.length) {
  if (originalCount !== 238) fail(`already-merged content count ${originalCount} !== 238`);
  console.log("merge-p6-maths: OK — already merged; no write needed.");
...
if (originalCount !== 214) fail(`original module count ${originalCount} !== 214`);
...
if (merged.modules.length !== 238) fail(`merged module count ${merged.modules.length} !== 238`);
...
console.log("merge-p6-maths: OK — merged.");
console.log(`  original count: ${originalCount}`);
