...
const minimumTextExplanationWords = 120;
const maximumTextExplanationWords = 170;
...
try {
  ...
  console.error(`check-p6-maths-structure: FAIL: ${error.message}`);
  process.exit(1);
}

if (!Array.isArray(source)) {
  console.error("check-p6-maths-structure: FAIL: source root is not an array.");
  process.exit(1);
}
...
source.forEach((module, index) => {
  ...
  const textExplanationWordCount =
    typeof module.textExplanationHa === "string" ? module.textExplanationHa.trim().split(/\s+/).filter(Boolean).length : 0;
  if (
    textExplanationWordCount < minimumTextExplanationWords ||
    textExplanationWordCount > maximumTextExplanationWords
  ) {
    addFailure(
      `${where}.textExplanationHa word count: expected ${minimumTextExplanationWords}-${maximumTextExplanationWords}, got ${textExplanationWordCount}`
    );
  }
...
if (failures.length > 0) {
  console.error(`check-p6-maths-structure: FAIL — ${failures.length} issue(s).`);
  failures.forEach((failure) => console.error(`- ${failure}`));
  process.exit(1);
}

console.log(
  "check-p6-maths-structure: PASS — 24 module(s), exact IDs p6-maths-01..24, schema fields, textExplanationHa word counts, micro-pauses, quiz counts, assets, and formal leaf fields verified."
);
